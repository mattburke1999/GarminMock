...
def register(firstname, lastname, email, username, password):
    ...
    #redirect(url_for('login')) if result[0] else render_template('registration.html', error=result[1])

def login(username, password):
    result = login_process(username, password)
    return jsonify(result[1], 200) if result[0] else jsonify(result[1], 400)
    #redirect(url_for('main.homePage_route')) if result[0] else render_template('login.html', show_error=result[1])
...
def searchByDate(date_str):
    ...
    return redirect(url_for('main.multiple_activity_route')) if result[0] else jsonify(result[1], 400)
    #redirect(url_for('main.multiple_activity_route')) if result[0] else render_template('searchByDate.html', error=result[1])
...
def searchByMonth(month, year):
    ...
    return redirect(url_for('main.multiple_activity_route')) if result[0] else jsonify(result[1], 400)
    #redirect(url_for('main.multiple_activity_route')) if result[0] else render_template('searchByMonth.html', error=result[1])

def multiple_activity():
    result = get_multiple_activity_info()
    if not result[0]:
        return jsonify(result[1], 400)
        #show_error_page(result[1], 'get_multiple_activity_info', 'Error getting activity information.')
    activity_list, lap_html_list, folium_maps = result[1]
    return jsonify({'activity_list': activity_list, 'lap_html_list': lap_html_list, 'folium_maps': folium_maps, 'date_title': session['date_title']}, 200)
    #render_template('multipleActivity.html', activity_list=activity_list, date_title = session['date_title'], lap_html_list=lap_html_list, folium_maps = folium_maps, zip=zip)
...
def activity(activity_id):
    ...
    return jsonify({'activity': activity_dict, 'lap_html': lap_html, 'folium_map': folium_map}, 200)
    #render_template('singleActivity.html', activity=activity_dict, lap_html=lap_html, folium_map=folium_map)
...
def get_calendar():
    ...
    #render_template('calendar.html', calendar=cal, calendar_title=month_string, month=month, year=year, current_month=current_month, sport='Other', zip=zip)
