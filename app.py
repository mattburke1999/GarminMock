...
## old functions based on SQL Server, will get commented/deleted as I replace them with Postgres functions

def connect_to_database():
    conn_str = os.environ.get('SQLServer')
...
def datetime_to_string(d):
    # converts timestamp to string in format 
    # Thursday January 4, 2024 @ 3:45 PM
    date_string = d.strftime('%A %B %d, %Y @ %I:%M %p')
    
    return date_string

def aggregate_activity_info_month(df):
    dict_list = []
    ...
    for sport in unique_sports:
        ...
        sport_dict = {
            'sport': sport,
            'num_activities': int(sport_df.iloc[0]['num_activities']),
            'total_distance': round(float(sport_df.iloc[0]['total_distance'])/1609.34,2),
            'total_time': str(time_seconds_to_string(sport_df.iloc[0]['total_time'])),
            'total_elapsed_time': str(time_seconds_to_string(sport_df.iloc[0]['total_elapsed_time'])),
            'avg_hr': int(sport_df.iloc[0]['avg_hr']),
            'max_heart_rate': int(sport_df.iloc[0]['max_heart_rate']),
            'total_calories': int(sport_df.iloc[0]['total_calories']),
            'avg_running_cadence': int(sport_df.iloc[0]['avg_running_cadence']),
            'max_running_cadence': int(sport_df.iloc[0]['max_running_cadence']),
            'total_strides': int(sport_df.iloc[0]['total_strides'])
        }
        dict_list.append(sport_dict)
    return dict_list   

def aggregate_activity_info_year(df):
    dict_list = []
    for sport in df['sport'].unique():
        sport_df = df[df['sport'] == sport]
        new_dict = {}
        new_dict['sport'] = sport
        new_dict['num_activities'] = int(sport_df['num_activities'])
        total_time = sport_df['total_time'].sum()
        new_dict['total_time'] = str(time_seconds_to_string(total_time))
        total_elapsed_time = sport_df['total_elapsed_time'].sum()
        new_dict['total_elapsed_time'] = str(time_seconds_to_string(total_elapsed_time))
        total_distance = float(sport_df['total_distance'].sum())
        # convert from meters to miles
        miles = total_distance / 1609.34
        new_dict['total_distance'] = float(round(miles, 2))
        hr_above_zero = sport_df[sport_df['avg_hr']>0]['avg_hr']
        if len(hr_above_zero) == 0:
            new_dict['avg_hr'] = 0
        else:
            new_dict['avg_hr'] = float(hr_above_zero.mean())
        new_dict['max_heart_rate'] = sport_df['max_heart_rate'].max()
        new_dict['total_calories'] = sport_df['total_calories'].sum()
        hr_above_zero = sport_df[sport_df['recovery_hr']>0]['recovery_hr']
        if len(hr_above_zero) == 0:
            new_dict['avg_recovery_hr'] = 0
        else:
            new_dict['avg_recovery_hr'] = float(hr_above_zero.mean())
        cadence_above_zero = sport_df[sport_df['avg_running_cadence']>0]['avg_running_cadence']
        if len(cadence_above_zero) == 0:
            new_dict['avg_running_cadence'] = 0
        else:
            new_dict['avg_running_cadence'] = cadence_above_zero.mean()
        new_dict['max_running_cadence'] = sport_df['max_running_cadence'].max()
        new_dict['total_strides'] = int(sport_df['total_strides'].sum())
        new_dict['total_calories'] = int(sport_df['total_calories'].sum())
        
        dict_list.append(new_dict)
    
    return dict_list    

def get_activity_info_by_month_summary(month, year):
    cnxn = connect_to_database()
    query = '{call dbo.get_activity_info_by_month_summary(?,?)}'
    activity_df = pd.read_sql_query(query, cnxn, params=[month, year])
    cnxn.close()
    if len(activity_df) == 0:
        return None
    activity_dict_list = aggregate_activity_info_month(activity_df)
    return activity_dict_list

def get_activity_info_by_year(year):
    cnxn = connect_to_database()
    query = "{call get_activity_info_by_year(?)}"
    activity_info_all = pd.read_sql_query(query, cnxn, params=[year])
    cnxn.close()
    if len(activity_info_all) == 0:
        return None
    activity_dict_list = aggregate_activity_info_year(activity_info_all)
    return activity_dict_list
    
    None

def aggregate_activity_info_month_by_week(df):
    if len(df) == 0:
        return None
    sports = []
    unique_sports = df['sport'].unique().tolist()
    for sport in unique_sports:
        sport_dict = {'sport': sport, 'weeks': []}
        sport_df = df[df['sport'] == sport]
        for week in range(len(sport_df)):
            week_dict = {
                'week_num': week + 1,
                'WeekStartDate': sport_df.iloc[week]['WeekStartDate'],
                'WeekEndDate': sport_df.iloc[week]['WeekEndDate'],
                'num_activities': int(sport_df.iloc[week]['num_activities']),
                'total_distance': round(float(sport_df.iloc[week]['total_distance'])/1609.34,2),
                'total_time': str(time_seconds_to_string(sport_df.iloc[week]['total_time'])),
                'total_elapsed_time': str(time_seconds_to_string(sport_df.iloc[week]['total_elapsed_time'])),
                'avg_hr': int(sport_df.iloc[week]['avg_hr']),
                'max_heart_rate': int(sport_df.iloc[week]['max_heart_rate']),
                'total_calories': int(sport_df.iloc[week]['total_calories']),
                'avg_running_cadence': int(sport_df.iloc[week]['avg_running_cadence']),
                'max_running_cadence': int(sport_df.iloc[week]['max_running_cadence']),
                'total_strides': int(sport_df.iloc[week]['total_strides'])
            }
            sport_dict['weeks'].append(week_dict)
        sports.append(sport_dict)
    return sports

def aggregate_activity_info_year_by_month(df):
    if len(df) == 0:
        return None
    sports = []
    unique_sports = df['sport'].unique().tolist()
    for sport in unique_sports:
        sport_dict = {'sport': sport, 'months': []}
        sport_df = df[df['sport'] == sport]
        for month in range(len(sport_df)):
            month_df = sport_df.iloc[month]['month']
            year = sport_df.iloc[month]['year']
            # convert from int to month name
            monthString = pd.to_datetime(f'{month_df}/01/{year}').strftime('%B')
            month_dict = {
                'monthString': f'{monthString} {year}',
                'num_activities': int(sport_df.iloc[month]['num_activities']),
                'total_distance': round(float(sport_df.iloc[month]['total_distance'])/1609.34,2),
                'total_time': str(time_seconds_to_string(sport_df.iloc[month]['total_time'])),
                'total_elapsed_time': str(time_seconds_to_string(sport_df.iloc[month]['total_elapsed_time'])),
                'avg_hr': int(sport_df.iloc[month]['avg_hr']),
                'max_heart_rate': int(sport_df.iloc[month]['max_heart_rate']),
                'total_calories': int(sport_df.iloc[month]['total_calories']),
                'avg_running_cadence': int(sport_df.iloc[month]['avg_running_cadence']),
                'max_running_cadence': int(sport_df.iloc[month]['max_running_cadence']),
                'total_strides': int(sport_df.iloc[month]['total_strides'])
            }
            sport_dict['months'].append(month_dict)
        sports.append(sport_dict)
    return sports

def getActivityInfoSummaryMonthByWeek(month, year):
    start_date = pd.Timestamp(f'{year}-{month}-01')
    end_date = pd.Timestamp(f'{year}-{month}-01') + pd.offsets.MonthEnd(0)
    # start_date, end_date = correctDatesForMonthByWeek(start_date, end_date)
    query = '{call get_activity_info_for_date_range_grouped_weekly(?, ?)}'
    cnxn = connect_to_database()
    df = pd.read_sql_query(query, cnxn, params=(start_date, end_date))
    cnxn.close()
    dict = aggregate_activity_info_month_by_week(df)
    return dict

def getActivityInfoSummaryYearByMonth(year):
    query = '{call get_activity_info_by_year_grouped_monthly(?)}'
    cnxn = connect_to_database()
    df = pd.read_sql_query(query, cnxn, params=[year])
    cnxn.close()
    dict = aggregate_activity_info_year_by_month(df)
    return dict

## NEW FUNCTIONS BASED ON POSTGRES
...
## NEW/UPDATED ROUTES BASED ON POSTGRES
...
## OLD ROUTES BASED ON SQL SERVER, WILL GET COMMENTED/DELETED AS I REPLACE THEM WITH POSTGRES ROUTES

@app.route('/monthGroupWeek', methods=['GET', 'POST'])
@login_required
def monthGroupWeek():
    return render_template('monthWeekGroup.html', sports=session['sport_list'], month_string=session['month'])

@app.route('/monthSummary', methods=['GET', 'POST'])
@login_required
def monthSummary():
    return render_template('monthSummary.html', data=session['dict_list'], month_string=session['month'], )

def pre_MonthSummary(month, year):
    dict_list = get_activity_info_by_month_summary(month, year)
    if dict_list is None:
        print("NO ACTIVITY FOUND")
        return render_template('searchByMonth.html')
    session['dict_list'] = dict_list
    # convert month from int to actual name
    month = pd.to_datetime(f'{month}/01/{year}').strftime('%B')
    month_string = f'{month} {year}'
    session['month'] = month_string
    print("ACTIVITY FOUND")
    return redirect(url_for('monthSummary'))

def pre_monthgroupweek(month, year):
    sport_list = getActivityInfoSummaryMonthByWeek(month, year)
    if sport_list is None:
        print("NO ACTIVITY FOUND")
        return render_template('searchByMonth.html')
    session['sport_list'] = sport_list
    # convert month from int to actual name
    month = pd.to_datetime(f'{month}/01/{year}').strftime('%B')
    month_string = f'{month} {year}'
    session['month'] = month_string
    print("ACTIVITY FOUND")
    return redirect(url_for('monthGroupWeek'))

def preYearSummary(year):
    dict_list = get_activity_info_by_year(year)
    if dict_list is None:
        print("NO ACTIVITY FOUND")
        return render_template('searchByYear.html')
    session['dict_list'] = dict_list
    year_string = str(year)
    session['year'] = year_string
    print("ACTIVITY FOUND")
    return redirect(url_for('yearSummary'))

def preYearRecap(year):
    dict_list = getActivityInfoSummaryYearByMonth(year)
    if dict_list is None:
        print("NO ACTIVITY FOUND")
        return render_template('searchByYear.html')
    session['dict_list'] = dict_list
    ...
    session['year'] = year_string
    print("ACTIVITY FOUND")
    return redirect(url_for('yearRecap'))
...
@app.route('/yearSummary', methods=['GET', 'POST'])
@login_required
def yearSummary():
    return render_template('yearSummary.html', data=session['dict_list'], year_string=session['year'])

@app.route('/yearRecap', methods=['GET', 'POST'])
@login_required
def yearRecap():
    return render_template('yearMonthGroup.html', sports=session['dict_list'], year_string=session['year'])
